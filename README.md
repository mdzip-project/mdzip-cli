...
```powershell
irm "https://raw.githubusercontent.com/mdzip-project/mdzip-cli/refs/heads/main/scripts/install.ps1?nocache=$(Get-Date -UFormat %s)" | iex
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/mdzip-project/mdzip-cli/refs/heads/main/scripts/install.sh | sudo sh
```

**Linux/macOS** (no sudo required, but you might need to fix PATH):

```bash
curl -fsSL https://raw.githubusercontent.com/mdzip-project/mdzip-cli/refs/heads/main/scripts/install.sh | sh
```
...
mdz create ./my-doc-folder my-doc.mdz --mode project --entry-point index.md
...
| `--mode` | `*` Sets `manifest.mode` to `document` or `project` |
...
- `manifest.mode` defaults to `document` when omitted; use `--mode project` for multi-document project archives.
...
```json
{
  ...
  "producer": {
    ...
    "core": {
      "name": "mdzip-core",
      "version": "<current mdzip-core version>",
...
`inspect` surfaces `manifest.spec.version` (falling back to legacy `mdz` if present), `manifest.mode`, and other key manifest metadata.
